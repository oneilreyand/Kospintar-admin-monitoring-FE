...
function MarketingHero({ hero }) {
  ...
  return (
    <SurfaceCard className="overflow-hidden border-slate-800 bg-slate-950 text-white">
      <div className="grid gap-8 px-6 py-6 lg:grid-cols-[1.35fr_0.65fr] lg:px-8">
        <div className="relative overflow-hidden rounded-[26px] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.22),transparent_35%),linear-gradient(135deg,#111827_0%,#0f172a_45%,#1d4ed8_100%)] p-6">
          <div className="absolute right-6 top-6 h-24 w-24 rounded-full bg-white/10 blur-2xl" />
          <div className="relative space-y-4">
            <StatusBadge tone="info">{safeHero.badge}</StatusBadge>
            <div>
              <h1 className="max-w-2xl text-3xl font-bold tracking-tight text-white lg:text-4xl">
                {safeHero.title}
              </h1>
        ...
        <div className="grid gap-4">
          {safeHero.highlights.map((item) => (
            <div
              key={item.label}
              className="rounded-[24px] border border-white/10 bg-white/5 p-5 backdrop-blur"
            >
              <p className="text-sm text-slate-400">{item.label}</p>
              <h3 className="mt-3 text-3xl font-bold tracking-tight text-white">{item.value}</h3>
